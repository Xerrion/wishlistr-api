...
from app import crud, auth, database, schemas

router = APIRouter(prefix="/wishes", tags=["wishes"])


@router.get("/", response_model=List[schemas.wish.Wish])
def read_wishes(skip: int = 0, limit: int = 100, db: Session = Depends(database.get_db)):
...
@router.put("/{wish_id}", response_model=schemas.wish.Wish)
def update_wish(
    wish_id: int,
    wish: schemas.wish.WishUpdate,
    db: Session = Depends(database.get_db),
    current_user: str = Depends(auth.get_current_user),
):
